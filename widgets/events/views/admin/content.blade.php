...
<table class="table table-bordered table-striped table-hover" id="{{ $field_name }}-holder">
	<thead>
		<tr>
			...
			<th>{{ trans('widget::global.event') }}</th>
...
<button type="button" class="btn btn-success btn-block btn-lg" onclick="openItemPopup()"><i class="fa fa-plus"></i> {{ trans('widget::global.add_event') }}</button>
...
function openItemPopup(data, i)
{
	...
	$modal.load(app_root + '/api/v1/widget/post/events/editEvent', {data: data, i: i, sl: "{{ $sl }}"}, function()
	{
		$modal.modal();
	});
}
...
function addRepeaterRow(field, action, data)
{
	...
	var thumb = (data.image != '') ? app_root + '/api/v1/thumb/nail?w=420&h=310&img=' + data.image : app_root + '/widgets/events/assets/img/empty.gif';
...
<tr id="row@{{i}}">
    ...
    <td>
		...
		<p class="text-muted"><span style="float:left;width:20px;text-align:center"> <i class="ion-calendar"></i></span> {@{{event_start}}} @{{#if event_end}}- {@{{event_end}}}@{{/if}}</p>
		<p class="text-muted"><span style="float:left;width:20px;text-align:center"> <i class="ion-ios-location"></i></span> @{{location}}</p>
    </td>
    <td class="text-center">
		<button type="button" data-i="@{{i}}" class="btn btn-info form-options" onclick="openItemPopup('@{{escape events}}', '@{{i}}')"><i class="fa fa-pencil"></i></button>
        <textarea name="{{ $field_name }}[]" class="form-control" id="events@{{i}}" style="display:none">@{{events}}</textarea>
    </td>
