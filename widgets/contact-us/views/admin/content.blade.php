...
$field_name = 'phone_number_btn';
$field_default_value = trans('widget::global.phone_number_btn_default');
$field_value = \Mobile\Controller\WidgetController::getData($page, $field_name, $field_default_value);
...
if($field_value != NULL)
{
	...
	data.title = '" . $field_value->title[$i] . "';
	data.value = '" . $field_value->value[$i] . "';";
...
<table class="table table-bordered table-striped table-hover" id="{{ $field_name }}-holder">
	<thead>
		<tr>
			...
			<th>{{ trans('widget::global.value') }}</th>
			<th></th>
		</tr>
	</thead>
    <tbody>
    ...
    <td>
		...
		<div class="btn-group" role="group">
			...
			<div class="input-group">
				...
				<input class="form-control" placeholder="{{ trans('widget::global.title') }}" value="@{{title}}" type="text" name="{{ $field_name }}[title][]">
			</div>

		</div>
	</td>
    <td style="min-width:200px">
		<input class="form-control" placeholder="{{ trans('widget::global.value') }}" type="text" value="@{{value}}" name="{{ $field_name }}[value][]">
    </td>
